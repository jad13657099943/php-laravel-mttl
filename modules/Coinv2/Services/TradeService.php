...
//            'state' => [CoinTrade::STATE_PROCESSING, CoinTrade::STATE_NOT_SUFFICIENT_FUNDS],
        ...
        if (!$trade) {
           ...
           /* $coinService = resolve(CoinService::class);
            $coin = $coinService->getBySymbol($withdraw->symbol, $options['coinOptions'] ?? []);
            switch ($coin->withdraw_state) {
                case 1:
                    $state = CoinWithdraw::STATE_HUMAN_TRANSFER;
                    break;
                case 2:
                    $state = CoinWithdraw::STATE_PROCESSING;
                    break;
                default:
                    break;
            }*/
     ...
     * @param CoinTrade $trade
     */
    public function processing(CoinTrade $trade)
    {
        $tokenIO = resolve(TokenIO::class);

        $coinConfig = CoinConfig::query()->where('symbol', $trade->symbol)
            ->where('chain', $trade->chain)
            ->first();
        if ($trade->symbol == 'USDT') {
            //usdt 要带上协议，最终为：TRC20.USDT | ERC20.USDT
            $symbol = $coinConfig->agreement;
        ...
        $res = $tokenIO->chainBalance($symbol, $trade->from);
        if ($res['code'] != 0) {
            return false;
        }
        $symbolBalance['balance'] = $res['data'] ?? 0; //链上余额
        if ($symbolBalance['balance'] >= $trade->num) { // 是否有余额转账
            // TODO 通知不足以转账

            $tokenIO = resolve(TokenIO::class);
            $result = $tokenIO->newWithdraw($trade->to, $trade->num, $symbol, $trade->id, $trade->memo,
                $trade->from, $trade->gas_price, $chain);
            if ($result['code'] == 0) {
                $trade->state = CoinTrade::STATE_BROADCASTED;
            }
        }
