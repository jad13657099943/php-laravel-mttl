...
use Modules\Coinv2\Components\Exchange\Manager as ExchangeManager;
use Modules\Coinv2\Components\TokenIO\TokenIO;
use Modules\Coinv2\Events\CoinPriceSynced;
...
class CoinService
{
        ...
        if (!empty($address)) {
            ...
            switch ($chain) {
                ...
                case 'ETH':
                    ...
                    break;
                case 'FIL':
                    $url = 'https://filfox.info/zh/message/' . $address;
                    break;
                default:
                    $url = '';
                    break;
     ...
     * @return bool
     */
    public function checkAddress($chain, $address, $exception = true)
    {
        /** @var TokenIO $tokenIO */
        $tokenIO = resolve(TokenIO::class);

        //$chain = Coin::query()->where('symbol', $symbol)->value('chain');
        $res = $tokenIO->chainCheck($chain, $address);

        if (!$res || $res['code'] != 0) {
            if ($exception) {
                throw new \Exception(trans('错误的地址:' . $address));
            }
