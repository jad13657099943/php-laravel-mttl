...
use Modules\Coin\Http\Requests\ExchangeOrderCreateRequest;
use Modules\Coin\Services\CoinService;
use Modules\Coin\Services\ExchangeService;
use Modules\Coin\Services\ExchangeSettingsService;
...
class ExchangeController
{
    ...
    public function createOrder(ExchangeOrderCreateRequest $request, ExchangeService $exchangeOrderService)
    {
        return $exchangeOrderService->create(array_merge([
            'user' => $request->user()
        ], $request->validationData()), ['checkPayPassword' => false]);
    }
